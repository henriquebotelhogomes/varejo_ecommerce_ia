...
(Gemini 3.8 Flash, DeepSeek V4.1 Flash) em comparação com modelos legados (GPT-4o).
"""
...
TOKEN_PRICING_PER_MILLION: dict[str, dict[str, float]] = {
    ...
    # Modelos Proprietários de Referência (Benchmark de Economia)
...
class FinOpsMetrics(BaseModel):
    ...
    projected_gpt4_cost_usd: float = Field(
        description="Custo estimado caso fosse executado no GPT-4o."
    )
    savings_usd: float = Field(description="Economia financeira gerada (GPT-4o - Custo Real).")
...
def auditar_finops(
    ...
    """Gera auditoria comparativa completa entre o modelo executado e a linha de base GPT-4o."""
    actual_cost = calcular_custo_tokens(prompt_tokens, completion_tokens, model_utilizado)
    gpt4_cost = calcular_custo_tokens(prompt_tokens, completion_tokens, "gpt-4o")
    savings = max(0.0, gpt4_cost - actual_cost)
    pct = (savings / gpt4_cost * 100) if gpt4_cost > 0 else 0.0
    ...
    return FinOpsMetrics(
        ...
        projected_gpt4_cost_usd=round(gpt4_cost, 6),
